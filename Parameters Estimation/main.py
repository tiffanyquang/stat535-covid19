...
beta_log_mean = np.nanmean(beta_log.values.flat)
beta_log_std = np.nanstd(beta_log.values.flat)
beta_log_sem = stats.sem(beta_log.values.flat, nan_policy='omit')
...
print('Estimate of log(beta):', format_sci(beta_log_mean, beta_log_sem))
print('Standard deviation:', beta_log_std)
...
y = np.exp(-(x-beta_log_mean)**2/(2*beta_log_std**2))/(np.sqrt(2*np.pi)*beta_log_std)
plt.plot(x, y)
plt.show()
